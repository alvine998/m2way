@extends('layouts.app', ['pageTitle' => __('app.add_category')])

@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('accounting-categories.store') }}" class="space-y-6">
        @csrf
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">{{ __('app.category_information') }}</h2>
            </div>
            <div class="p-6 space-y-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" required
                           ...
                           placeholder="e.g., Tiket Pesawat, Hotel, Konsumsi">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="icon" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.icon_emoji') }}</label>
                    ...
                    <div>
                        <label for="color" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.color') }}</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="sort_order" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.sort_order') }}</label>
        ...
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('accounting-categories.index') }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">{{ __('app.cancel') }}</a>
            <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-brand rounded-lg hover:bg-brand/90 transition-colors shadow-sm">{{ __('app.save_category') }}</button>
        </div>
    </form>
</div>
