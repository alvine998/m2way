@extends('layouts.app', ['pageTitle' => __('app.edit_transaction')])
        ...
        <div class="space-y-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="px-6 py-4 border-b border-gray-100">
                    <h2 class="text-lg font-semibold text-gray-900">{{ __('app.transaction_details') }} - {{ $transaction->transaction_number }}</h2>
                </div>
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-search-select name="customer_id" :value="old('customer_id', $transaction->customer_id)" label="{{ __('app.customer') }}" placeholder="{{ __('app.select_customer') }}" required
                    ...
                    <div>
                        ...
                        <select name="package_id" id="package_id" x-model="packageId" @change="updatePackageInfo()" required
                            ...
                            <option value="{{ $pkg->id }}" data-price="{{ $pkg->base_price }}" data-name="{{ $pkg->name }}" data-type="{{ $pkg->type }}" {{ old('package_id', $transaction->package_id) == $pkg->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="departure_date" id="departure_date" value="{{ old('departure_date', $transaction->departure_date?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="confirmed" {{ old('status', $transaction->status) === 'confirmed' ? 'selected' : '' }}>{{ __('app.confirmed') }}</option>
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="payment_type" id="payment_type" x-model="paymentType" required
                            ...
                            <option value="cicilan" {{ old('payment_type', $transaction->payment_type) === 'cicilan' ? 'selected' : '' }}>{{ __('app.cicilan_installment') }}</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="payment_method" id="payment_method" required
                            ...
                            <option value="cash" {{ old('payment_method', $transaction->payment_method) === 'cash' ? 'selected' : '' }}>{{ __('app.tunai_cash') }}</option>
                            <option value="transfer" {{ old('payment_method', $transaction->payment_method) === 'transfer' ? 'selected' : '' }}>{{ __('app.bank_transfer') }}</option>
                            <option value="credit_card" {{ old('payment_method', $transaction->payment_method) === 'credit_card' ? 'selected' : '' }}>{{ __('app.credit_card') }}</option>
                            <option value="debit_card" {{ old('payment_method', $transaction->payment_method) === 'debit_card' ? 'selected' : '' }}>{{ __('app.debit_card') }}</option>
                            <option value="ewallet" {{ old('payment_method', $transaction->payment_method) === 'ewallet' ? 'selected' : '' }}>{{ __('app.e_wallet') }}</option>
                        </select>
                    ...
                    <div x-show="paymentType === 'cicilan'" x-transition class="md:col-span-2 p-4 bg-amber-50 border border-amber-200 rounded-lg space-y-4">
                        ...
                        @else
                        <p class="text-xs text-amber-600">{{ __('app.payment_already_recorded') }}</p>
                        @endif
                    </div>

                    <div class="md:col-span-2">
                        <label for="payment_status" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.payment_status') }} <span class="text-red-500">*</span></label>
                        <select name="payment_status" id="payment_status" required
                                class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all bg-white @error('payment_status') border-red-300 @enderror">
                            <option value="unpaid" {{ old('payment_status', $transaction->payment_status) === 'unpaid' ? 'selected' : '' }}>{{ __('app.unpaid') }}</option>
                            <option value="partial" {{ old('payment_status', $transaction->payment_status) === 'partial' ? 'selected' : '' }}>{{ __('app.partial') }}</option>
                            <option value="paid" {{ old('payment_status', $transaction->payment_status) === 'paid' ? 'selected' : '' }}>{{ __('app.paid') }}</option>
                        </select>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="discount" id="discount" value="{{ old('discount', $transaction->discount) }}" min="0" x-model.number="discount" @input="calculateTotal()" placeholder="0"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                <div class="p-6">
                    ...
                    <textarea name="notes" id="notes" rows="3" placeholder="Catatan"
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all resize-none @error('notes') border-red-300 @enderror">{{ old('notes', $transaction->notes) }}</textarea>
            ...
            <div class="flex items-center justify-end space-x-3">
                ...
                <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-brand rounded-lg hover:bg-brand/90 transition-colors shadow-sm">{{ __('app.update_transaction') }}</button>
            </div>
        </div>
    </form>
</div>
...
function transactionForm() {
    return {
        ...
        packageId: '{{ old("package_id", $transaction->package_id) }}',
        packagePrice: {{ $transaction->total_price ?? 0 }},
        packageName: '{{ $transaction->package->name ?? "" }}',
        packageType: '{{ $transaction->package->type ?? "" }}',
        discount: {{ old('discount', $transaction->discount) }},
        grandTotal: {{ $transaction->grand_total ?? 0 }},
        paymentType: '{{ old("payment_type", $transaction->payment_type ?? "cash") }}',
        init() {
