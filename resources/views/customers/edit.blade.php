@extends('layouts.app', ['pageTitle' => __('app.edit_customer')])
    ...
    <form method="POST" action="{{ route('customers.update', $customer) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">{{ __('app.edit_customer') }} - {{ $customer->name }}</h2>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $customer->name) }}" placeholder="Nama lengkap" required
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $customer->email) }}" placeholder="email@example.com"
                    ...
                    <div>
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $customer->phone) }}" placeholder="08xxxxxxxxxx" required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address" id="address" rows="3" placeholder="Alamat lengkap"
                                  class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all resize-none @error('address') border-red-300 @enderror">{{ old('address', $customer->address) }}</textarea>
                        @error('address')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_type" id="id_type"
                            ...
                            <option value="nik" {{ old('id_type', $customer->id_type) === 'nik' ? 'selected' : '' }}>{{ __('app.ktp') }}</option>
                            <option value="passport" {{ old('id_type', $customer->id_type) === 'passport' ? 'selected' : '' }}>{{ __('app.passport') }}</option>
                        </select>
                        @error('id_type')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="id_number" id="id_number" value="{{ old('id_number', $customer->id_number) }}" placeholder="No. KTP/Paspor"
                    ...
                    <div>
                        ...
                        <input type="text" name="nationality" id="nationality" value="{{ old('nationality', $customer->nationality) }}" placeholder="Indonesia"
                    ...
                    <div>
                        ...
                        <input type="text" name="occupation" id="occupation" value="{{ old('occupation', $customer->occupation) }}" placeholder="Pekerjaan"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="3" placeholder="Catatan tambahan"
                                  class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all resize-none @error('notes') border-red-300 @enderror">{{ old('notes', $customer->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">{{ __('app.customer_attachments') }}</h2>
                <p class="text-sm text-gray-500 mt-1">{{ __('app.ktp_required') }} {{ __('app.passport_required') }}</p>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- KTP -->
                    <div>
                        @php $ktp = $customer->attachments->where('description', 'KTP')->first(); @endphp
                        ...
                        @if($ktp)
                            <div class="mb-3 p-3 bg-gray-50 rounded-lg flex items-center gap-3">
                                @if($ktp->isImage())
                                    <img src="{{ $ktp->url }}" alt="KTP" class="w-12 h-12 rounded object-cover">
                                @else
                                ...
                                @endif
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-700">{{ $ktp->file_name }}</p>
                                    <p class="text-xs text-gray-400">{{ $ktp->formatted_size }}</p>
                                </div>
                                <span class="text-xs text-green-600 font-medium">{{ __('app.uploaded') }}</span>
                            </div>
                        @endif
                        <div class="border-2 border-dashed {{ $ktp ? 'border-gray-200' : 'border-gray-300' }} rounded-lg p-4 text-center hover:border-brand transition-colors"
                            ...
                            <template x-if="!file">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">{{ $ktp ? __('app.replace_file') : __('app.drop_files_here') }} <label class="relative text-brand font-medium cursor-pointer hover:underline">{{ __('app.browse') }}<input type="file" name="ktp" x-ref="ktpInput" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" accept=".jpg,.jpeg,.png,.pdf" @change="file = $el.files[0]"></label></p>
                    ...
                    <!-- Passport -->
                    <div>
                        @php $passport = $customer->attachments->where('description', 'Passport')->first(); @endphp
                        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('app.passport') }}</label>
                        @if($passport)
                            <div class="mb-3 p-3 bg-gray-50 rounded-lg flex items-center gap-3">
                                @if($passport->isImage())
                                    <img src="{{ $passport->url }}" alt="Passport" class="w-12 h-12 rounded object-cover">
                                @else
                                ...
                                @endif
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-gray-700">{{ $passport->file_name }}</p>
                                    <p class="text-xs text-gray-400">{{ $passport->formatted_size }}</p>
                                </div>
                                <span class="text-xs text-green-600 font-medium">{{ __('app.uploaded') }}</span>
                            </div>
                        @endif
                        <div class="border-2 border-dashed {{ $passport ? 'border-gray-200' : 'border-gray-300' }} rounded-lg p-4 text-center hover:border-brand transition-colors"
                             x-data="{ dragging: false, file: null }"
                             x-on:dragover.prevent="dragging = true"
                             x-on:dragleave.prevent="dragging = false"
