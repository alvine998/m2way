...
@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('package-categories.update', $category) }}" class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" value="{{ old('color', $category->color) }}" id="color_text"
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <input type="color" id="bg_color" name="bg_color" value="{{ old('bg_color', $category->bg_color) }}"
                            class="w-10 h-10 rounded-lg border border-gray-300 cursor-pointer p-0.5" />
                        <input type="text" value="{{ old('bg_color', $category->bg_color) }}" id="bg_color_text"
                            class="flex-1 rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm font-mono focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none"
            ...
            <div>
                ...
                <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <span id="badge-preview" class="inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold"
                          style="background-color: {{ old('bg_color', $category->bg_color) }}; color: {{ old('color', $category->color) }};">
                        <span id="preview-icon">{{ old('icon', $category->icon) }}</span><span id="preview-name">{{ old('name', $category->name) }}</span>
                    </span>
                </div>
            </div>
