@extends('layouts.app', ['pageTitle' => __('app.edit_package')])

@section('content')
<div class="max-w-4xl" x-data="packageForm(@js($package->cost_details ?? []))">
    <form method="POST" action="{{ route('packages.update', $package) }}">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $package->name) }}" placeholder="Nama Paket" required
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->slug }}" {{ old('type', $package->type) === $cat->slug ? 'selected' : '' }}>{{ $cat->icon ? $cat->icon . ' ' : '' }}{{ $cat->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="duration" id="duration" value="{{ old('duration', $package->duration) }}" placeholder="Hari" min="1" required
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="draft" {{ old('status', $package->status) === 'draft' ? 'selected' : '' }}>{{ __('app.draft') }}</option>
                            <option value="active" {{ old('status', $package->status) === 'active' ? 'selected' : '' }}>{{ __('app.active') }}</option>
                            <option value="inactive" {{ old('status', $package->status) === 'inactive' ? 'selected' : '' }}>{{ __('app.inactive') }}</option>
                        </select>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Deskripsi paket"
                                  class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all resize-none @error('description') border-red-300 @enderror">{{ old('description', $package->description) }}</textarea>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="date" name="departure_date" id="departure_date" value="{{ old('departure_date', $package->departure_date?->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="return_date" id="return_date" value="{{ old('return_date', $package->return_date?->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="departure_city" id="departure_city" value="{{ old('departure_city', $package->departure_city) }}" placeholder="Kota keberangkatan"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" inputmode="numeric" name="base_price" id="base_price" value="{{ old('base_price', $package->base_price) }}" placeholder="Harga dasar" min="0" required
                    ...
                    <div>
                        ...
                        <input type="number" name="quota" id="quota" value="{{ old('quota', $package->quota) }}" placeholder="Jumlah kursi" min="1" required
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="hotel_name" id="hotel_name" value="{{ old('hotel_name', $package->hotel_name) }}" placeholder="Nama hotel"
                    ...
                    <div>
                        ...
                        <input type="text" name="airline" id="airline" value="{{ old('airline', $package->airline) }}" placeholder="Nama maskapai"
                    ...
                    <div>
                        ...
                        <textarea name="includes" id="includes" rows="3" placeholder="{{ __('app.one_item_per_line') }}"
                                  class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all resize-none @error('includes') border-red-300 @enderror">{{ old('includes', is_array($package->includes) ? implode("\n", $package->includes) : $package->includes) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="excludes" id="excludes" rows="3" placeholder="{{ __('app.one_item_per_line') }}"
                                  class="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand/20 focus:border-brand outline-none transition-all resize-none @error('excludes') border-red-300 @enderror">{{ old('excludes', is_array($package->excludes) ? implode("\n", $package->excludes) : $package->excludes) }}</textarea>
            ...
            <div class="flex items-center justify-end space-x-3">
                ...
                <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-brand rounded-lg hover:bg-brand/90 transition-colors shadow-sm">{{ __('app.update_package_button') }}</button>
            </div>
        </div>
    </form>
</div>
...
<script>
function packageForm(existingCosts = []) {
    const items = existingCosts.length > 0
        ? existingCosts.map(c => ({ label: c.label || '', amount: c.amount || 0 }))
        : [{ label: '', amount: 0 }];
