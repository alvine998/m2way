@extends('layouts.app', ['pageTitle' => __('app.edit_schedule')])

@section('content')
<div class="space-y-6">
    <form method="POST" action="{{ route('plan-schedules.update', $planSchedule) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="package_id" id="package_id" required
                        ...
                        @foreach($packages as $package)
                            <option value="{{ $package->id }}" {{ old('package_id', $planSchedule->package_id) == $package->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="planned_date" id="planned_date"
                        value="{{ old('planned_date', $planSchedule->planned_date->format('Y-m-d')) }}" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none">
                ...
                <div>
                    ...
                    <input type="number" name="target_pax" id="target_pax"
                        value="{{ old('target_pax', $planSchedule->target_pax) }}" min="1" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none"
                        placeholder="Jumlah">
                ...
                <div>
                    <label for="booked_pax" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.booked_pax') }}</label>
                    <input type="number" name="booked_pax" id="booked_pax"
                        value="{{ old('booked_pax', $planSchedule->booked_pax) }}" min="0"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none"
                        placeholder="Jumlah">
                    @error('booked_pax') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none">
                        <option value="planned" {{ old('status', $planSchedule->status) === 'planned' ? 'selected' : '' }}>{{ __('app.planned') }}</option>
                        <option value="active" {{ old('status', $planSchedule->status) === 'active' ? 'selected' : '' }}>{{ __('app.active') }}</option>
                        <option value="completed" {{ old('status', $planSchedule->status) === 'completed' ? 'selected' : '' }}>{{ __('app.completed') }}</option>
                        <option value="cancelled" {{ old('status', $planSchedule->status) === 'cancelled' ? 'selected' : '' }}>{{ __('app.cancelled') }}</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:border-brand focus:ring-2 focus:ring-brand/20 outline-none"
                        placeholder="{{ __('app.notes_optional') }}">{{ old('notes', $planSchedule->notes) }}</textarea>
