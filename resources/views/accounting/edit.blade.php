@extends('layouts.app', ['pageTitle' => __('app.edit_accounting_entry')])

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100">
        <div class="px-6 py-4 border-b border-gray-100">
            <h2 class="text-lg font-semibold text-gray-900">{{ __('app.edit_entry') }} - {{ $accounting->entry_number }}</h2>
        </div>
        <form method="POST" action="{{ route('accounting.update', $accounting) }}" enctype="multipart/form-data" class="p-6" x-data="{ type: '{{ old('type', $accounting->type) }}' }">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <!-- Type -->
                <div>
                    ...
                    <div class="flex space-x-4">
                        <label class="flex items-center px-4 py-3 border border-gray-200 rounded-lg cursor-pointer hover:border-brand transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/5">
                            <input type="radio" name="type" value="income" {{ old('type', $accounting->type) === 'income' ? 'checked' : '' }} class="text-brand focus:ring-brand" required @change="type = 'income'">
                            <span class="ml-2 text-sm font-medium text-gray-700">{{ __('app.income') }}</span>
                        </label>
                        <label class="flex items-center px-4 py-3 border border-gray-200 rounded-lg cursor-pointer hover:border-brand transition-colors has-[:checked]:border-brand has-[:checked]:bg-brand/5">
                            <input type="radio" name="type" value="expense" {{ old('type', $accounting->type) === 'expense' ? 'checked' : '' }} class="text-brand focus:ring-brand" required @change="type = 'expense'">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="booking_income" {{ old('category', $accounting->category) === 'booking_income' ? 'selected' : '' }}>{{ __('app.booking_income') }}</option>
                        <option value="refund" {{ old('category', $accounting->category) === 'refund' ? 'selected' : '' }}>{{ __('app.refund') }}</option>
                        <option value="operational_cost" {{ old('category', $accounting->category) === 'operational_cost' ? 'selected' : '' }}>{{ __('app.operational_cost') }}</option>
                        <option value="salary" {{ old('category', $accounting->category) === 'salary' ? 'selected' : '' }}>{{ __('app.salary') }}</option>
                        <option value="marketing" {{ old('category', $accounting->category) === 'marketing' ? 'selected' : '' }}>{{ __('app.marketing') }}</option>
                        <option value="office_cost" {{ old('category', $accounting->category) === 'office_cost' ? 'selected' : '' }}>{{ __('app.office_cost') }}</option>
                        <option value="transport" {{ old('category', $accounting->category) === 'transport' ? 'selected' : '' }}>{{ __('app.transport') }}</option>
                        <option value="visa_fee" {{ old('category', $accounting->category) === 'visa_fee' ? 'selected' : '' }}>{{ __('app.visa_fee') }}</option>
                        <option value="miscellaneous" {{ old('category', $accounting->category) === 'miscellaneous' ? 'selected' : '' }}>{{ __('app.miscellaneous') }}</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description" value="{{ old('description', $accounting->description) }}" required
                ...
                <div>
                    <label for="amount" class="block text-sm font-medium text-gray-700 mb-1">{{ __('app.amount_rp') }} <span class="text-red-500">*</span></label>
                        <input type="text" inputmode="numeric" name="amount" id="amount" value="{{ old('amount', $accounting->amount) }}" min="0" required
                ...
                <div>
                    ...
                    <input type="date" name="date" id="date" value="{{ old('date', $accounting->date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              ...
                              placeholder="Catatan">{{ old('notes', $accounting->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div x-transition>
                    ...
                    @php $existingAttachments = $accounting->attachments ?? []; @endphp
                    @if(!empty($existingAttachments))
                    <div class="mb-3 p-3 bg-gray-50 rounded-lg flex items-center gap-3">
                        @php $att = $existingAttachments[0]; @endphp
                        @if(in_array(strtolower(pathinfo($att['file_name'] ?? '', PATHINFO_EXTENSION)), ['jpg','jpeg','png']))
                            <img src="{{ asset('storage/' . ($att['file_path'] ?? '')) }}" alt="evidence" class="w-12 h-12 rounded object-cover">
                        @else
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-400">{{ isset($att['size']) ? round($att['size'] / 1024, 1) . ' KB' : '' }}</p>
                        </div>
            ...
            <div class="flex items-center justify-end space-x-3 mt-8 pt-6 border-t border-gray-100">
                ...
                <button type="submit" class="px-5 py-2.5 text-sm font-semibold text-white bg-brand rounded-lg hover:bg-brand/90 transition-colors shadow-sm">{{ __('app.update_entry') }}</button>
            </div>
        </form>
    </div>
</div>
