@extends('layouts.app', ['pageTitle' => __('app.add_document')])

@section('content')
<div class="max-w-4xl">
    <form method="POST" action="{{ route('travel-documents.store') }}" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">{{ __('app.travel_document_management') }}</h2>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <x-search-select name="customer_id" :value="old('customer_id')" label="{{ __('app.customer') }}" placeholder="{{ __('app.customer') }}" required
                ...
                <div>
                    ...
                    <select name="document_type" id="document_type" required
                        ...
                        <option value="passport" {{ old('document_type') === 'passport' ? 'selected' : '' }}>{{ __('app.passport') }}</option>
                        <option value="ktp" {{ old('document_type') === 'ktp' ? 'selected' : '' }}>{{ __('app.ktp') }}</option>
                        <option value="visa" {{ old('document_type') === 'visa' ? 'selected' : '' }}>{{ __('app.visa') }}</option>
                        <option value="vaccination" {{ old('document_type') === 'vaccination' ? 'selected' : '' }}>{{ __('app.vaccination') }}</option>
                        <option value="insurance" {{ old('document_type') === 'insurance' ? 'selected' : '' }}>{{ __('app.insurance') }}</option>
                        <option value="ticket" {{ old('document_type') === 'ticket' ? 'selected' : '' }}>{{ __('app.ticket') }}</option>
                        <option value="hotel" {{ old('document_type') === 'hotel' ? 'selected' : '' }}>{{ __('app.hotel') }}</option>
                ...
                <div>
                    ...
                    <input type="text" name="issuing_country" id="issuing_country" value="{{ old('issuing_country', 'Indonesia') }}"
