@extends('layouts.app', ['pageTitle' => __('app.add_group')])

@section('content')
<div class="max-w-4xl">
    <form method="POST" action="{{ route('jamaah-groups.store') }}" class="space-y-6">
        @csrf
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-900">{{ __('app.jamaah_group_management') }}</h2>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="package_id" id="package_id" required
                        ...
                        <option value="{{ $pkg->id }}" {{ old('package_id') == $pkg->id ? 'selected' : '' }}>{{ $pkg->name }} ({{ strtoupper($pkg->type) }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    <x-search-select name="leader_id" :value="old('leader_id')" label="{{ __('app.group_leader') }}" placeholder="{{ __('app.select_leader') }}"
                ...
                <div>
                    ...
                    <input type="number" name="quota" id="quota" value="{{ old('quota', 40) }}" min="1" max="100" required
